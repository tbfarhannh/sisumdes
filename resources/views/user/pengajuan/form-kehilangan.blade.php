...
<section>
    <div class="flex items-center gap-1.5">
        ...
        <p class="text-gray-600 text-sm mt-0.5">Form Surat Keterangan Kehilangan.</p>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-300 p-6 mt-6">
        ...
        <form action="{{ route('user.pengajuan.kehilangan') }}" method="POST" class="space-y-5">
            ...
            <div>
                <label for="barang_hilang" class="block font-semibold text-var7 mb-1.5">Barang yang Hilang</label>
                <input type="text" id="barang_hilang" name="barang_hilang" value="{{ old('barang_hilang') }}" class="w-full px-5 py-3 border border-gray-300 rounded-lg focus:ring focus:ring-var1 focus:border-var1 outline-none transition" required placeholder="Contoh: KTP, STNK, Ijazah">
            </div>
    
            <div>
                <label for="keterangan" class="block font-semibold text-var7 mb-1.5">Keterangan (Tempat/Waktu Kehilangan)</label>
                <textarea id="keterangan" name="keterangan" rows="3" class="w-full px-5 py-3 border border-gray-300 rounded-lg focus:ring focus:ring-var1 focus:border-var1 outline-none transition" required>{{ old('keterangan') }}</textarea>
            </div>
